...
Specialist personas that play a single role with a single perspective. Each persona is a Markdown file consumed as a system prompt by your agent harness (OpenCode, Cursor, Copilot, etc.).
...
This is a useful orchestration pattern when the investigations are independent. See [references/orchestration-patterns.md](../references/orchestration-patterns.md) for the full pattern catalog and anti-patterns.
...
2. **Personas do not invoke other personas unless the active harness explicitly supports bounded delegation.** Composition remains the responsibility of the task owner, which must define scope, authority, and merge ownership.
3. A persona may invoke skills (the *how*).
4. Every persona file ends with a "Composition" block stating where it fits.

## OpenCode interop

The personas in this repo work as OpenCode subagents without modification:

- **As subagents:** Use the `task` tool with an appropriate persona description to invoke a specialist perspective. The persona file defines the role, scope, and output format.
- **As skills:** Personas may be referenced in `AGENTS.md` for automatic invocation during relevant workflows (e.g., `code-reviewer` before merge).

Subagents report results back to the main agent. Use them when you need a focused, single-perspective analysis without cross-agent coordination.
