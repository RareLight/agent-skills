...
Cross-session citation cache for [`source-driven-development`](../skills/source-driven-development/SKILL.md). Skips redundant fetch calls without weakening the skill's "verify against current docs" guarantee.
...
1. Register the hooks in your agent harness's settings file (e.g. `.harness/settings.json`, `.claude/settings.json`):
...
The stored body is not raw HTML — the fetch tool post-processes each response through a model, so what we cache is one agent's reading of the page. The key stays URL-only so reads reuse across sessions; the original prompt is kept as metadata and surfaced in the hit message so the next agent can tell whether the earlier reading fits.

## How it works

One cache entry per URL, stored as JSON in a project-local cache directory:

| Event | Action |
|---|---|
| `PreToolUse (fetch)` | If an entry exists, sends a `HEAD` request with `If-None-Match` / `If-Modified-Since`. On `304`, blocks the fetch and returns the cached content via stderr, with the original prompt surfaced as metadata. Otherwise allows the fetch. |
| `PostToolUse (fetch)` | Captures the response, issues a `HEAD` request to record the current `ETag` / `Last-Modified`, and stores the entry. |

**Freshness rules:**

- Entry is served only if the origin confirms `304 Not Modified`.
- Entries without an `ETag` or `Last-Modified` header are never cached — without a validator, the hook cannot verify freshness later.
...
- Cache hit: the fetch is blocked via exit code 2. The harness delivers the hook's stderr payload back to the agent as a tool error — this is the intended signal for a cache hit, not a failure. The payload is prefixed with `[sdd-cache] Cache hit for <url>` and wraps the cached body between `----- BEGIN CACHED CONTENT -----` / `----- END CACHED CONTENT -----` markers so the agent can use it as if the fetch had just returned it.
- Cache miss or stale: the fetch runs normally; the result is stored for next time.
...
- First command creates one file under the cache directory (only if the server returned an `ETag` or `Last-Modified`).
...
1. Register the hooks in your harness settings as shown above.
2. Start an agent session in this repo.
3. Ask the agent to fetch a documentation page (e.g. "fetch `https://react.dev/reference/react/useActionState` and summarize").
4. Verify a file appears under the cache directory.
5. Ask the agent to fetch the same page with the same prompt again.
6. Verify the second fetch is blocked and the cached content is returned (visible in the session transcript as a tool error with `[sdd-cache]` prefix).

### 3. Freshness verification

To confirm the cache invalidates when docs change, force an `ETag` mismatch:
...
Both hooks write timestamped events to a debug log when debug mode is on. Enable it with either:

```bash
# Option A: env var (per-session)
SDD_CACHE_DEBUG=1 <agent-command>
...
The log captures URL, detected response shape, HEAD status, and why each invocation hit or missed.

## Known limitations

- **Body is prompt-shaped.** A hit returns the earlier agent's reading of the page, with the original prompt surfaced so the current agent can decide whether it applies. If it doesn't, delete the cache entry to force a re-fetch.
- **Every cache write costs an extra HEAD.** The harness doesn't expose the response headers from the fetch tool, so the post hook re-queries the origin to capture `ETag` / `Last-Modified`. One extra roundtrip per miss — the price of keeping this a pure hook with no core changes.
