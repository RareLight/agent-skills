...
Reference catalog of agent orchestration patterns this repo endorses, plus anti-patterns to avoid. Read this before adding a new workflow that coordinates multiple personas, or before introducing a new persona that "wraps" existing ones.
...
**Cost:** same as direct invocation. The command is just a saved instruction.
...
The user runs commands in a defined order, carrying context between them. There is no orchestrator agent — the user IS the orchestrator.
...
**Implementation:** Use a read-only sub-agent (no write/edit tools, no skill invocation). Most agent harnesses support spawning restricted sub-agents. If your harness has a built-in research sub-agent (e.g. OpenCode's `task` tool with `explore` subagent type), prefer that over a custom one.
...
**What to do instead:** keep the user as the orchestrator. Document the recommended sequence in `README.md` and let the user invoke commands.
...
Most agent harnesses support sub-agent spawning and context isolation. Key differences:

| Capability | Notes |
|------------|-------|
| **Parallel sub-agents** | Some harnesses require multiple tool calls in one turn; others serialize. Check your harness docs before adopting Pattern 3. |
| **Read-only sub-agents** | Most harnesses support restricting tool access. Pattern 5 (research isolation) depends on this. |
| **Agent-to-agent messaging** | The Adversarial Debugging pattern (not in this catalog) requires teammates that message each other. Only some harnesses support this. |
| **Hook systems** | Pre/post tool hooks are harness-specific. Check your harness's settings format. |
| **Plugin vs. project-local** | Some harnesses discover agents from a plugin directory; others from the project root. Configure accordingly. |

The five patterns above are harness-agnostic. Adapt the implementation details to your specific agent harness — the orchestration logic stays the same.
